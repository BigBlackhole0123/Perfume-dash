...
# เชื่อมต่อ Google Sheets อัตโนมัติผ่าน Secrets ของ Streamlit
conn = st.connection("gsheets", type=GSheetsConnection)

# ดึงข้อมูลจาก Sheets ออกมาแสดงผล
try:
...
try:
    ...
    exp_db = pd.DataFrame(columns=["วันที่", "รายการ", "จำนวนเงิน", "หมายเหตุ"])

# --- ส่วนคำนวณแดชบอร์ด ---
total_sales = pd.to_numeric(sales_df["รวมยอดขาย"]).sum() if not sales_df.empty else 0
total_cogs = pd.to_numeric(sales_df["รวมต้นทุน"]).sum() if not sales_df.empty else 0
gross_profit = total_sales - total_cogs
total_expenses = pd.to_numeric(exp_df["จำนวนเงิน"]).sum() if not exp_df.empty else 0
...
st.markdown("---")
st.subheader("💰 การจัดสรรกระแสเงินสด (เงินหมุนเวียนธุรกิจ)")
...
with tab2:
    with st.form("exp_form", clear_on_submit=True):
        ...
        if submit_exp and e_title and e_amount > 0:
            ...
            st.success("บันทึกรายจ่ายลง Google Sheets เรียบร้อยแล้ว!")
            st.rerun()

st.subheader("📋 รายการปัจจุบันที่ดึงมาจากฐานข้อมูล")
